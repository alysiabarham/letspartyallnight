  ...
  socket.on("submit_item", (data) => {
    ...
    if (playerAlreadySubmitted) {
      socket.emit("submission_error", {
        message: "You have already submitted an item for this round.",
      });
      return;
    }
    ...
    console.log(
      `Player ${room.players[socket.id].name} submitted item in room ${roomCode}: "${item}"`,
    );
    ...
    if (room.submittedItems.length === Object.keys(room.players).length) {
      ...
      console.log(
        `Ranking phase started in room ${roomCode}. Ranking player: ${rankingPlayerName}`,
      );
    }
  });
  ...
  socket.on("submit_secret_ranking", (data) => {
    ...
    if (roomCode && gameRooms[roomCode]) {
      ...
      if (socket.id === room.rankingPlayerId && room.gameStatus === "ranking") {
        ...
        if (
          submittedItemIdsSet.size !== rankedItemIds.length || // Check for duplicates in submission
          ![...submittedItemIdsSet].every((id) => originalItemIds.has(id))
        ) {
          // Check if all submitted IDs are from original items
        ...
        console.log(
          `Secret ranking submitted by ${room.players[socket.id].name} in room ${roomCode}:`,
          room.secretRanking,
        );
        ...
        if (playersToGuess.length > 0) {
          ...
          io.to(roomCode).emit("no_guessers_ending_round", {
            message: "No other players to guess. Round ending.",
          });
          room.gameStatus = "results"; // Temporarily move to results
        }
      } else {
        socket.emit("secret_ranking_error", {
          message: "You are not the designated ranking player or game is not in ranking phase.",
        });
      }
    } else {
  ...
  socket.on("submit_guess", (data) => {
    ...
    if (socket.id === room.rankingPlayerId) {
      socket.emit("guess_error", {
        message: "You are the ranking player; you cannot submit a guess.",
      });
      return;
    }
    ...
    if (room.playerGuesses[socket.id]) {
      socket.emit("guess_error", {
        message: "You have already submitted your guess for this round.",
      });
      return;
    }
    ...
    if (
      submittedItemIdsSet.size !== guessedItemIds.length ||
      ![...submittedItemIdsSet].every((id) => originalItemIds.has(id))
    ) {
    ...
    room.guessCount++;
    socket.emit("guess_submitted_confirmation", {
      message: "Your guess has been submitted! Waiting for others...",
    });
    console.log(
      `Player ${room.players[socket.id].name} submitted guess in room ${roomCode}. Guess count: ${
        room.guessCount
      }`,
    );
    ...
    if (roomCode) {
      ...
      if (room.hostId === socket.id) {
        ...
        if (remainingPlayerIds.length > 0) {
          ...
          console.log(
            `New host for room ${roomCode}: ${room.players[newHostId].name} (${newHostId})`,
          );
      ...
      if (room.gameStatus === "guessing_phase" && socket.id !== room.rankingPlayerId) {
        ...
        const playersToGuess = Object.values(room.players).filter(
          (p) => p.id !== room.rankingPlayerId,
        );
        // Check if all *remaining* guessers have submitted
        if (room.guessCount === playersToGuess.length) {
          console.log(
            `All remaining guesses submitted in room ${roomCode}. Proceeding to results.`,
          );
          room.gameStatus = "results";
          io.to(roomCode).emit("all_guesses_submitted", {
