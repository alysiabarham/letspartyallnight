...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("start_ranking_phase", (data) => {
      ...
      const itemsWithIds = data.itemsToRank.map((item) => ({
        ...item,
    ...
    socket.on("start_guessing_phase", (data) => {
      ...
      const itemsWithIds = data.itemsToGuess.map((item) => ({
        ...item,
        id: item.id || generateUniqueId(),
      })); // Ensure ID
    ...
    socket.on("guess_submitted_confirmation", (data) => {
      // NEW: Confirmation for guess submission
    ...
    socket.on("guess_error", (data) => {
      // NEW: Error for guess submission
      setLobbyMessage(`Guess Error: ${data.message}`);
      console.error("Guess Error:", data.message);
    });

    socket.on("all_guesses_submitted", (data) => {
      // NEW: All guesses are in, time for results
  ...
  const handleSubmitSecretRanking = () => {
    if (
      currentRoomCode &&
      socket.id === rankingPlayerId &&
      !secretRankingSubmitted &&
      isConnected
    ) {
      ...
      <div className="App">
        <header className="App-header">
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            />
            <button onClick={handleJoinRoom} disabled={!playerName.trim() || !inputRoomCode.trim()}>
              Join Room
            </button>
          </div>
                ...
                <ul>
                  {receivedMessages.map((msg, index) => (
                    <li key={index} style={{ listStyleType: "none", marginBottom: "5px" }}>
                      <strong>[{msg.senderId ? msg.senderId.substring(0, 4) : "Server"}]:</strong>{" "}
    ...
    return (
      <div className="App">
        <header className="App-header">
          ...
          >
            <h3>Topic:</h3>
            <p style={{ fontSize: "1.5em", fontWeight: "bold", color: "#61dafb" }}>
              {currentTopic}
            </p>
            {!submissionConfirmed ? (
              <>
        ...
        <header className="App-header">
          ...
          <div style={{ marginTop: "30px", borderTop: "1px solid #ccc", paddingTop: "20px" }}>
            ...
            {isThisPlayerRanking && !secretRankingSubmitted ? (
              <>
                <p>
                  You are the **RANKING PLAYER**! Drag and drop to order these items from 1 (top) to{" "}
                  {itemsToRank.length} (bottom):
                </p>
                <DndContext
                  sensors={sensors}
              ...
              <p>
                You have submitted your secret ranking. Waiting for other players to submit their
                guesses...
              </p>
            ) : (
              <>
                <p>
                  <strong>{rankingPlayerName}</strong> is secretly ranking the items.
                </p>
        ...
        <header className="App-header">
          ...
          <div style={{ marginTop: "30px", borderTop: "1px solid #ccc", paddingTop: "20px" }}>
            ...
            {isThisPlayerRanking ? (
              ...
              <>
                <p>
                  <strong>{rankingPlayerName}</strong> has submitted their secret ranking.
                </p>
                <p>
                  Now, it's your turn to drag and drop these items into the order you think they
                  picked!
                </p>
